...
import { IPlaceDetails, PlaceDetailsResponse } from '@/interfaces/location';
...
export const getPlaceDetail = async (placeId: string): Promise<PlaceDetailsResponse> => {
    const url = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${placeId}&key=${process.env.EXPO_PUBLIC_GOOGLE_MAPS_API_KEY}&fields=name,address_components,formatted_address,rating,user_ratings_total,website,formatted_phone_number,geometry,photos,types`;
 ...
 */
const mapPlaceDetails = (place: any, placeId: string): IPlaceDetails => {
    const photos = place.photos ? place.photos.map((photo: any) => `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${photo.photo_reference}&key=${process.env.EXPO_PUBLIC_GOOGLE_MAPS_API_KEY}`) : [];
